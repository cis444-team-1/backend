...
func InitRoutes(e *echo.Echo, h *handlers.Handler) {
	...
	e.GET("/tracks/:trackId", h.GetTrackByTrackIDHandler) // Get specific track by its id
	...
	e.GET("/playlists/:playlistId", h.GetPlaylistHandler)                       // Get specific playlist by its id
	...
	e.GET("/playlists/:playlistId/tracks", h.GetTracksFromPlaylistHandler)      // Get songs in playlist by playlist id
	e.POST("/playlists/:playlistId/tracks", h.AddTrackToPlaylistHandler)        // Add song to playlist, Requires middleware
